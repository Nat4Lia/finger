...
class GaugeDraw(ImageDraw.ImageDraw):
    ...
    def draw_needle(self):
        """Draws the needle"""
        # w_gauge_value = self.gauge_value - 1
        # if w_gauge_value < self.min_value:
        #     w_gauge_value = self.min_value

        # if w_gauge_value > self.max_value:
        #     w_gauge_value = self.max_value

        # angle = math.radians(self.min_angle + (w_gauge_value - self.min_value) *
        ...
        # self._thick_polygon((left_point, end_point, right_point, mid_point), outline=0,
        #                 fill=0, thickness=self.dial_thickness)
