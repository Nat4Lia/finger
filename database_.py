    ...
    def sendLogToServer(self):
        ...
        if self.koneksifinger & self.koneksiinternet :
            ...
            if self.checkMac:
                ...
                totalUpload = 0
                if selisih > 0:
                    lcd_.printLCD('%s Data Record' % selisih,'Ditemukan').lcd_status()
                    for data in range (jumlahLogLocal, jumlahDataLog):
                        ...
                        if requestPOST(URL['ATTENDACE'], headers, payload).status_code is 200:
                            ...
                            totalUpload += 1
                            lcd_.printLCD('Updating...','%s Data' % totalUpload).lcd_status()
                            ...
                            if totalUpload == selisih :
                                ...
                                self.cursor.close()
                                self.cnx.close()
                                lcd_.printLCD('Success Updating','%s Data' % totalUpload).lcd_status()
